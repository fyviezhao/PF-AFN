...
## Our Environment
Change Log: To support more recent GPUs (RTX 3090), we SYSU-OpenFashion update the environment with pytorch 1.7.1, cuda 11.0 and cupy 9.1.0. Note we have **not** yet double-checked whether the FID score remians 9.92 on the VITON test set after transfer to the new environment. **As suggested by the author, one should better to use the original environment (which is indicated below) to run the test specifically for the FID score verification.**

anaconda3

pytorch 1.7.1 (original 1.1.0)

torchvision 0.8.2 (original 0.3.0)

cuda 11.0 (original 9.0)

cupy 9.1.0 (original 6.0.0)

opencv-python 4.5.2.54 (original 4.5.1)

(oritinal 8 GTX1080) GPU for training; 1 GTX3090 (original 1 GTX1080) GPU for test

python 3.8.10 (original 3.6)

## Installation
```sh
conda create -n afpfn python=3.8

conda activate afpfn
...
git clone https://github.com/sysu-openfashion/PF-AFN.git
...
## Run the demo
1. cd PF-AFN_test
2. First, you need to download the checkpoints from [checkpoints](https://drive.google.com/file/d/1_a0AiN8Y_d_9TNDhHIcRlERz3zptyYWV/view?usp=sharing) and put the folder "PFAFN" under the folder "PF-AFN_test/checkpoints". The folder "PF-AFN_test/checkpoints/PFAFN" shold contain "warp_model_final.pth" and "gen_model_final.pth". 
3. The "demo_dataset" folder contains the demo images for test, where the "test_img" folder contains the person images, the "test_clothes" folder contains the clothes images, and the "test_edge" folder contains edges extracted from the clothes images with the built-in function in python (We saved the extracted edges from the clothes images for convenience). 'demo.txt' records the test pairs. 
...
5. To test with the saved model on the demo dataset, run **demo.sh** and the results will be saved in the folder "results/demo/PFAFN".
...
## Testing on VITON dataset
1. cd PF-AFN_test
2. First, you need to download the checkpoints from [checkpoints](https://drive.google.com/file/d/1_a0AiN8Y_d_9TNDhHIcRlERz3zptyYWV/view?usp=sharing) and put the folder "PFAFN" under the folder "PF-AFN_test/checkpoints". The folder "PF-AFN_test/checkpoints/PFAFN" shold contain "warp_model_final.pth" and "gen_model_final.pth". 
3. Download the VITON test set from [VITON_test](https://drive.google.com/file/d/1Y7uV0gomwWyxCvvH8TIbY7D9cTAUy6om/view?usp=sharing). and put the folder "VITON_test" under the folder "PF-AFN_test". The folder "PF-AFN_test/VITON_test" should contain three subfolders including "test_clothes", "test_edge" and "test_img". The "VITON_test.txt" records the test pairs of VITON dataset. 
3. To test with the saved model on the VITON dataset, run test_VITON.sh and the results will be saved in the folder "results/test_VITON/PFAFN".
4. **To reproduce the exact same results as reported in the original paper from the saved model, your test environment should be the same as the original test environment, especifically for the version of cupy.** 
...
6. Following the above insructions with the provided training code, the [[trained PF-AFN]](https://drive.google.com/file/d/1Pz2kA65N4Ih9w6NFYBDmdtVdB-nrrdc3/view?usp=sharing) achieves FID 9.92 on VITON test set with the test_pairs.txt
